...
<div class="space-y-6">
    ...
    <div class="p-4 rounded-2xl bg-white dark:glass-card border border-gray-200 dark:border-white/10">
        <form method="GET" action="{{ route('mechanic.inventory.index') }}" class="flex flex-col md:flex-row flex-wrap items-stretch md:items-center gap-3">
            ...
            @if($search || ($category && $category !== 'all') || $stockStatus)
                <a href="{{ route('mechanic.inventory.index') }}" class="px-4 py-2.5 rounded-xl border border-gray-300 dark:border-white/10 text-gray-700 dark:text-white/70 hover:bg-gray-100 dark:hover:bg-white/10 text-sm font-medium flex items-center justify-center gap-2 whitespace-nowrap">
